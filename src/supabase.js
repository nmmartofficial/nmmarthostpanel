...
const enhanceWithMaybeSingle = (queryBuilder) => {
  ...
  return new Proxy(queryBuilder, {
    get(target, prop, receiver) {
      if (prop === 'maybeSingle') {
        return async function maybeSingleFallback() {
          try {
            const result = await target.single()
            if (result && result.error && (
              result.error.code === 'PGRST116' ||
              (typeof result.error.message === 'string' && result.error.message.includes('contains 0 rows'))
            )) {
            ...
            if (err && typeof err.message === 'string' && err.message.includes('contains 0 rows')) {
              return { data: null, error: null }
            }
            throw err
          }
      ...
      if (typeof value === 'function') {
        return function (...args) {
          ...
          if (result && typeof result === 'object') {
            return enhanceWithMaybeSingle(result)
          }
          return result
        }
      }
...
const wrapSupabaseFrom = (client) => {
  const originalFrom = client.from.bind(client)
  return Object.assign({}, client, {
    from: (tableName, options) => {
      const queryBuilder = originalFrom(tableName, options)
      return enhanceWithMaybeSingle(queryBuilder)
    }
  })
}
...
if (useMock) {
  ...
  try {
    ...
    supabaseInstance = wrapSupabaseFrom(rawClient)
