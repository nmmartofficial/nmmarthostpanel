          ...
          <table className="w-full text-left border-collapse min-w-[1400px]">
            <thead className="sticky top-0 z-10 bg-slate-50 shadow-sm">
              <tr className="border-b border-slate-200">
                <th className="px-4 py-3 text-[11px] font-black text-slate-800 uppercase tracking-widest">SNo</th>
                <th className="px-4 py-3 text-[11px] font-black text-slate-800 uppercase tracking-widest">Name / Category</th>
                <th className="px-4 py-3 text-[11px] font-black text-slate-800 uppercase tracking-widest text-center">Barcode / HSN</th>
                <th className="px-4 py-3 text-[11px] font-black text-slate-800 uppercase tracking-widest text-center">Brand / Counter</th>
                <th className="px-4 py-3 text-[11px] font-black text-slate-800 uppercase tracking-widest text-center">Pricing (₹)</th>
                <th className="px-4 py-3 text-[11px] font-black text-slate-800 uppercase tracking-widest text-center">Tax / Disc</th>
                <th className="px-4 py-3 text-[11px] font-black text-slate-800 uppercase tracking-widest text-center">Stock / Specs</th>
                <th className="px-4 py-3 text-[11px] font-black text-slate-800 uppercase tracking-widest text-center">Picture</th>
                <th className="px-4 py-3 text-[11px] font-black text-slate-800 uppercase tracking-widest text-right">Action</th>
              </tr>
            </thead>
              ...
              {paginatedProducts.map((product, idx) => {
                ...
                <tr key={product.id} className="odd:bg-white even:bg-slate-100 hover:bg-blue-50/30 transition-colors group">
                  ...
                  <td className="px-4 py-3">
                    <p className="text-[11px] font-black text-slate-800 uppercase tracking-tighter leading-tight">{productName}</p>
                    <p className="text-[9px] font-bold text-slate-400 mt-1 uppercase">
                      {resolveCategoryName(product)} {resolveSubcategoryName(product) !== '-' ? `/ ${resolveSubcategoryName(product)}` : ''}
                    </p>
                  </td>
                  <td className="px-4 py-3 text-center">
                    <p className="text-[11px] font-bold text-slate-600">{product.barcode || '-'}</p>
                    <p className="text-[9px] font-bold text-slate-400 uppercase">HSN: {getVal('hsncode', 'hsn_code') || '-'}</p>
                  </td>
                  <td className="px-4 py-3 text-center">
                    <p className="text-[11px] font-bold text-slate-600 uppercase">{resolveBrandName(product)}</p>
                    <p className="text-[9px] font-bold text-slate-400 uppercase">{product.counter_name || '-'}</p>
                  </td>
                  <td className="px-4 py-3 text-center">
                    <div className="flex flex-col items-center">
                      <span className="text-[11px] font-black text-blue-700">Sale: ₹{getVal('onlinerate', 'sale_rate')}</span>
                      <span className="text-[9px] text-slate-400 line-through font-bold">MRP: ₹{product.mrp}</span>
                      <span className="text-[9px] text-slate-500 font-bold">Purc: ₹{getVal('purcrate', 'purchase_rate') || 0}</span>
                    </div>
                  </td>
                  <td className="px-4 py-3 text-center">
                    <div className="flex flex-col items-center gap-1">
                      <span className="bg-amber-100 text-amber-700 px-2 py-0.5 rounded-full text-[9px] font-black uppercase tracking-widest">
                        {getVal('discperc', 'discount_percent') || 0}% OFF
                      </span>
                      <span className="text-[9px] font-bold text-slate-500 uppercase">GST: {getVal('gst', 'gst_percent') || 0}%</span>
                    </div>
                  </td>
                  <td className="px-4 py-3 text-center">
                    <div className="flex flex-col items-center">
