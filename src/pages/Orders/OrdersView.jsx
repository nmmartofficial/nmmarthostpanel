...
const fetchStoredOrderItems = async (orderId) => {
  /*
   * 1. Directly read orders.items.
   * This is the current canonical snapshot for website orders.
   */
  try {
  ...
  /*
   * 2. dbSync fallback for orders.items.
   */
  try {
    ...
    console.error(
      'Synced orders.items fallback failed:',
      error
    );
  }

  /*
   * 3. Legacy order_items fallback.
   */
  try {
  ...
  /*
   * 4. Direct Supabase order_items fallback.
   */
  try {
    const {
...
export default function OrdersView({
    ...
    const loadOrderItemSummaries =
      async () => {
        ...
        for (const order of currentOrders) {
          ...
          /*
           * Convert snapshot into readable summary.
           */
          const summary = items
            .map((item) => {
              const name =
  ...
  const fetchOrderItems = async (
    ...
    try {
      /*
       * First use selected order's JSON snapshot.
       */
      let sourceItems =
        parseStoredOrderItems(
          selectedOrder?.items
      ...
      /*
       * Product IDs for optional product-master
       * fallback data such as image/name/rate.
       */
      const productIds = [
        ...new Set(
          sourceItems
  ...
  const handleReturnOrder =
    async (e) => {
      ...
      try {
        ...
        /*
         * orders table does not have
         * return_reason / return_amount.
         *
