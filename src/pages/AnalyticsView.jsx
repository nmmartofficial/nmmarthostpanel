    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
          <h3 className="text-xs font-black text-slate-800 uppercase tracking-widest mb-6">Order Delivery Status</h3>
          <div className="h-[300px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie
        ...
        <div className="lg:col-span-2 bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
          <h3 className="text-xs font-black text-slate-800 uppercase tracking-widest mb-6">Top Performing Categories</h3>
          <div className="h-[300px] w-full">
            <ResponsiveContainer width="100%" height="100%">
        ...
        <div className="lg:col-span-3 bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
          <div className="flex items-center justify-between mb-6">
            ...
            <button className="text-[9px] font-black text-blue-600 uppercase tracking-widest hover:underline">Download Report</button>
          </div>
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="border-b border-slate-100 bg-slate-50/50">
                  ...
                  <th className="px-4 py-3 text-[9px] font-black text-slate-400 uppercase tracking-widest text-center">Category</th>
                  <th className="px-4 py-3 text-[9px] font-black text-slate-400 uppercase tracking-widest text-center">Price</th>
                  <th className="px-4 py-3 text-[9px] font-black text-slate-400 uppercase tracking-widest text-center">Stock</th>
                  <th className="px-4 py-3 text-[9px] font-black text-slate-400 uppercase tracking-widest text-right">Performance</th>
                </tr>
              </thead>
                ...
                {products.slice(0, 5).map((p, i) => (
                  <tr key={i} className="hover:bg-slate-50/50 transition-colors">
                    <td className="px-4 py-3">
                      ...
                      <p className="text-[8px] font-bold text-slate-400 uppercase mt-0.5">ID: {p.id.slice(0, 8)}</p>
                    </td>
                    <td className="px-4 py-3 text-center">
                      <span className="text-[9px] font-black text-slate-600 uppercase bg-slate-100 px-2 py-0.5 rounded-full">{p.category_name || 'N/A'}</span>
                    </td>
                    <td className="px-4 py-3 text-center text-[10px] font-black text-slate-800">₹{p.sale_rate}</td>
                    <td className="px-4 py-3 text-center">
                      <span className={cn(
                    ...
                    <td className="px-4 py-3 text-right">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        <span className="text-[9px] font-black text-slate-400">{80 - (i * 10)}%</span>
                      </div>
                    </td>
                  </tr>
                ))}
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex flex-col">
          ...
          <div className="space-y-6 flex-1 flex flex-col justify-center">
            {[
              { label: 'Low Stock Items', count: products.filter(p => p.stock > 0 && p.stock <= 5).length, color: 'bg-amber-500' },
          ...
          <div className="mt-6 pt-6 border-t border-slate-100 flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] font-black text-slate-500 uppercase">Total SKUs</span>
            </div>
