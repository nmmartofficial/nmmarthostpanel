      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            exit={{ opacity: 0, y: 10 }}
            className={cn('absolute left-0 mt-2 bg-white rounded-xl shadow-[0_10px_40px_-10px_rgba(0,0,0,0.2)] border border-slate-200 py-2 z-[200] overflow-hidden w-64')}
          >
            <div className={cn('grid divide-slate-100 max-h-[85vh] overflow-y-auto grid-cols-1 divide-y')}>
              {items.map((item) => (
                <button
                  key={item.id}
                  onClick={() => {
