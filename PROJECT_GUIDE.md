...
## 15. Product Module - Critical Implementation Details

### Current Product Database Schema
This is the exact schema for the `products` table in Supabase (as of July 2026):
```sql
create table public.products (
   id bigint generated by default as identity not null,
   itname text null,
   itnameprint text null,
   barcode text null,
   imagename text null,
   itemdescription text null,
   hsncode text null,
   picture text null,
   takerate numeric(10,2) null,
   restrate numeric(10,2) null,
   dlvrate numeric(10,2) null,
   onlinerate numeric(10,2) null,
   purcrate numeric(10,2) null,
   mrp numeric(10,2) null,
   opstock numeric(10,2) null,
   discperc numeric(5,2) null,
   isfav boolean null default false,
   unitcode text null,
   itg text null,
   itc text null,
   dtcode text null,
   kcode text null,
   brandcode text null,
   isdiscountable boolean null default true,
   gst numeric(5,2) null,
   cess numeric(5,2) null,
   shopid text null,
   ispackage boolean null default false,
   narration text null,
   narration2 text null,
...
### Strict Product Column Whitelist
To prevent errors like `PGRST204 (Column not found)`, we enforce a strict column whitelist for product operations!
Whitelisted columns in `dbSync.js`:
- `itname`, `itnameprint`, `barcode`, `imagename`, `itemdescription`
- `hsncode`, `picture`, `takerate`, `restrate`, `dlvrate`, `onlinerate`
- `purcrate`, `mrp`, `opstock`, `discperc`, `isfav`
- `unitcode`, `itg`, `itc`, `dtcode`, `kcode`, `brandcode`
- `isdiscountable`, `gst`, `cess`, `shopid`, `ispackage`
- `narration`, `narration2`, `itemstatus`

### Product ID
- **DO NOT MANUALLY SET PRODUCT ID**: The `id` column is a `bigint` auto-incrementing identity column managed by Supabase!
- Code in `ProductsView.jsx` and `dbSync.js` no longer uses UUIDs!
- When inserting a new product via `dbSync.insert()`, it returns the inserted data (including the auto-generated id), which we use for inventory logs!

### `is_active` Column Note
- The `products` table **does NOT have an `is_active` column**!
- `products` is REMOVED from `TABLES_WITH_IS_ACTIVE` array in `dbSync.js`!
- If you add `is_active` to products in the future, remember to add it back!

### Boolean Type Handling
Excel/CSV import automatically converts text values to proper booleans for these fields:
- `isfav`: true if text is "true", "yes", or "1"
- `isdiscountable`: true if text is NOT "false", "no", or "0"
- `ispackage`: true if text is "true", "yes", or "1"

---

## 16. Git & Deployment
- GitHub Repo: https://github.com/nmmartofficial/nmmarthostpanel
- Main Branch: `main` (always keep this stable!)
- Deployment: Vercel (configured via `vercel.json`)
- Commit Messages: Use clear, concise messages (e.g., `fix: remove UUID for product ID`)
